...
function renderProducts(products){
    ...
    products.forEach(product => {
        ...
        if (product.status === "unavailable") {
            ...
            stockMessage = `<p class="out-stock-text"> Currently Unavailable  </p>  `;
            ...
            stockMessage = ` <p class="out-stock-text">  Out Of Stock  </p>  `;

        }

        else if (product.stock <= 5) {

            stockMessage = `  <p class="low-stock-text"> Hurry up! Only ${product.stock} left </p> `;
        }

        card.innerHTML = ` <img src="${product.image}" alt="${product.title}"  class="product-img" />
            ...
            ${
                out_of_stock
                ? `<div class="product-overlay">Out of Stock</div>`
                : ""
            }

            ${
                unavailable
                ? `<div class="product-overlay">Unavailable</div>`
                : ""
            }
...
async function renderCart(cart) {
    ...
    cart.items.forEach(item => {
        ...
        if (item.unavailableReason === "removed") {
            overlayText = "Not Available";
        }

        else if (item.unavailableReason === "not_available") {
            overlayText = "Currently Unavailable";
        }

        else if (item.unavailableReason === "out_of_stock") {
            ...
            <div class="cart-item ${unavailable ? "not-available" : ""}">
                ...
                <div class="cart-info">
                    ...
                    ${
                        ...
                        : `
                            <div>
                                <button class="decrease" data-id="${item._id}">
                                    -
                                </button>

                                <span>${item.quantity}</span>

                                <button class="increase" data-id="${item._id}">
                                    +
                                </button>
                            </div>
                            <p>
